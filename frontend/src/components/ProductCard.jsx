...
const ProductCard = ({product}) => {
    ...
    return product &&(
        <div onClick={()=> {navigate(`/product/${product.category.toLowerCase()}/${product._id}`); scrollTo(0,0)}} className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white min-w-40 max-w-46 w-full">
            ...
            <div className="text-gray-500/60 text-sm">
                ...
                <div className="flex items-center gap-0.5">
                    {Array(5).fill('').map((_, i) => (
                        ...
                        <img src={i < 4 ? assets.star_icon : assets.star_dull_icon} alt="star" key={i} className="md:w-3.5 w-3"/>
                        ) 
                    )}
                    <p>(4)</p>
                </div>
