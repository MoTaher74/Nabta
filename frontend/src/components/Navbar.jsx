...
const Navbar = () => {
    ...
    return (
        <nav className="flex items-center justify-between px-6 md:px-16 lg:px-24 xl:px-32 py-4 border-b border-gray-300 bg-white relative transition-all">
            ...
            <NavLink to="/">
                <img className="h-18 w-18" src={assets.logoicon} alt="logo" />
            </NavLink>
            ...
            <div className="hidden sm:flex items-center gap-8">
                ...
                {/* {user && <NavLink to={'/order'} onClick={()=> setOpen(false)}>My Orders</NavLink>} */}
    ...
    {       !user ?    ( <button onClick={()=> setShowUserLogin(true)} className="cursor-pointer px-8 py-2 bg-primary hover:bg-secondry transition text-white rounded-full">
                    ...
                    <div className="relative group">
                        <img src={assets.profile_icon} alt="profilelogo" className="w-10"/>
                        <ul className="hidden group-hover:block absolute top-10 right-0 bg-white shadow border border-gray-300 py-2.5 w-30 rounded-md text-sm z-40">
                            <li className="p-1.5 pl-3 hover:bg-primary/10 cursor-pointer" onClick={()=> navigate("order")}>My Orders</li>
            ...
            { open && (<div className={`${open ? 'flex' : 'hidden'} absolute top-[60px] z-50 left-0 w-full bg-white shadow-md py-4 flex-col items-center gap-2 px-5 text-sm md:hidden`}>
                ...
                {!user ? (
                    ...
                    }} className="cursor-pointer px-6 py-2 mt-2 bg-primary hover:bg-secondry transition text-white rounded-full text-sm">
                    Login
                </button>
            )
            :
            (
                <button onClick={logout} className="cursor-pointer px-6 py-2 mt-2 bg-primary hover:bg-secondry transition text-white rounded-full text-sm">
                Logout
            </button> 
            )}
