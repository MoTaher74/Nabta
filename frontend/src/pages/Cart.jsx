        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                ...
                <button onClick={()=> {navigate('/product'); scrollTo(0,0);} } className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                    <img src={assets.arrow_right_icon_colored} alt="arrow" className=" transition group-hover:translate-x-1" />
                    Continue Shopping
                </button>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <div className="mb-6">
                    ...
                    <div className="relative flex justify-between items-start mt-2">
                        <p className="text-gray-500">{selectedAddress ? ` ${selectedAddress.city}, ${selectedAddress.street},${selectedAddress.state},${selectedAddress.country}` :"No address found"}</p>
                        <button onClick={() => setShowAddress(!showAddress)} className="text-indigo-500 hover:underline cursor-pointer">
                            Change
                        </button>
                        {showAddress && (
                            <div className="absolute top-12 py-1 bg-white border border-gray-300 text-sm w-full">
                               {address.map((address,idx) =>(<p key={idx} onClick={() => {setSelectedAddress(address) ; setShowAddress(false)}} className="text-gray-500 p-2 hover:bg-gray-100">
                ...
                <div className="text-gray-500 mt-4 space-y-2">
                    ...
                    <p className="flex justify-between">
                        <span>Tax (2%)</span><span>${Math.floor(getCartTotalAmount() * 0.02)}</span>
                    </p>
                    <p className="flex justify-between text-lg font-medium mt-3">
                        <span>Total Amount:</span><span>${Math.floor(getCartTotalAmount() + getCartTotalAmount() * 0.02)}</span>
                    </p>
                </div>
